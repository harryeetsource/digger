...
bool is_suspicious(const MemoryRegion & region) {
  ...
  if (is_executable(region) && (region.allocation_protect & PAGE_NOCACHE) == 0 && (region.state_flags & MEM_MAPPED) == 0 && (region.allocation_protect & MEM_IMAGE) == 0) {
    return true;
  }
  // Check if the region is writable and not a file-backed mapping
  if (is_writable(region) && (region.allocation_protect & PAGE_NOCACHE) == 0 && (region.state_flags & MEM_MAPPED) == 0 && (region.allocation_protect & MEM_IMAGE) == 0) {
    return true;
  }
  return false;
}
